...
include("include/connection.php");
    ...
    $result1 = mysql_query("SELECT title, content FROM flats3")or die(mysql_error());
    $numRows = mysql_num_rows($result1);
    
    if($numRows<10 && $numRows!=0)
    {
        $a = $numRows-5;
        $result = mysql_query("SELECT title, content FROM flats3 ORDER by id ASC LIMIT $a ")or die(mysql_error());
    ...
    elseif($numRows>10)
    {
        ...
        for($i=0;$i<=4;$i++)
        {
           $result = mysql_query("SELECT * FROM flats3  WHERE id='$start' ORDER by id DESC ")or die(mysql_error());
