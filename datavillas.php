...
include("include/connection.php");
    ...
    $result1 = $mysqli->query("SELECT title, content FROM villas")or die($mysqli->error);
    $numRows = $result1->num_rows;
    
    if($numRows<10 && $numRows!=0)
    {
        $a = $numRows-5;
        $result = $mysqli->query("SELECT title, content FROM villas ORDER by id ASC LIMIT $a ")or die($mysqli->error);
    ...
    elseif($numRows>10)
    {
        ...
        for($i=0;$i<=4;$i++)
        {
           $result = $mysqli->query("SELECT * FROM villas  WHERE id='$start' ORDER by id DESC ")or die($mysqli->error);
